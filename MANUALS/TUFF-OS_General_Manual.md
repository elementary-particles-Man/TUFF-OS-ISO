...
### 2.2 Runtime-Managed Asynchronous Runtime
The OS core operates on a runtime-managed asynchronous scheduler that keeps the hot path bounded and avoids unbounded allocation during boot-critical work. It does not monopolize the CPU even under heavy I/O or network attacks. By processing tasks efficiently in the background, it reduces the risk of system-wide resource exhaustion.
...
- **Isolation Mode**: Immediate transition upon 3 consecutive forged tokens. Sensitive state is zeroized promptly and the system fails closed.
