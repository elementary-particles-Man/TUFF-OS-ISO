...
flowchart TD
    ...
    class E fail
```
TUFF-Core asynchronous processing uses runtime-managed scheduling and fixed-size session/state structures on the hot path. Task waking is performed via `AtomicU32` bitmap notifications, resuming tasks in O(1) constant time from an interrupt (IRQ). This keeps the core responsive while the queueing and writeback layers handle bulk work.
...
- **PQC (Post-Quantum Cryptography) Audit**: Discarded packet logs are hashed and signed using ML-DSA family quantum-resistant signatures, creating an audit trail that is designed to be tamper-evident.
