...
| **Runtime-Managed Session State** | Session management keeps active sessions in fixed slots and zeroizes sensitive state promptly upon Isolation. | Zero memory residue. |
...
| **PQC Signature Audit** | All discarded packets are recorded with ML-DSA quantum-resistant signatures + hash chains. | Audit trails are tamper-proof. |
