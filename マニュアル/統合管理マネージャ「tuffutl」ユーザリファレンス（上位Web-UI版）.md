...
**tuffutl Web-UI** は、アプリケーション層（Windows / TUFF-KERNEL / macOS）からTUFF-OSを直感的に操作するための**ローカル専用ブラウザインターフェース**です。

- **設計思想**: 物理層の状態を可視化しつつ、**アプリケーション層からは一切直接触れさせない**  
- **動作方式**: アプリケーション層上でローカルサーバー（`tuffwin` / `tufflnx` / `tuffmac`）が起動し、ブラウザから `127.0.0.1:8080` にアクセス  
- **セキュリティ**: 外部ネットワークからは一切アクセス不可（localhost限定）。通信はすべてKEY-CSEで暗号化

```mermaid
flowchart TD
    A[アプリケーション層ブラウザ] --> B[localhost:8080]
    B --> C[tuffutl Web-UI]
    C --> D[ZRAM IPC]
    D --> E[TUFF-OS物理層]
...
- **ユーザーID**: TUFF-OS独自のID（アプリケーション層のユーザー名とは別）
