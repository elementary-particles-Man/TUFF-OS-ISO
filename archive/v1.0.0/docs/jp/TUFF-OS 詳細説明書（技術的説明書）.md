...
flowchart TD
    ...
    class E fail
```
TUFF-Coreの非同期処理は、動的メモリ確保（ヒープアロケーション）を一切行いません。タスクの起床（Wake）は `AtomicU32` のビットマップ通知によって行われ、割り込み（IRQ）からO(1)の定数時間でタスクを再開します。これにより、極限の高負荷時でもOOM（メモリ枯渇）やカーネルパニックを回避します。
...
- **PQC（ポスト量子暗号）証跡**: 破棄したパケットのログは、MlDsa44を用いた量子耐性署名によってハッシュチェーン化され、改ざん不能な証跡として保存されます。
