...
2. 3回目：即時Isolationモード移行
   - ZRAM上のセッション鍵・TagGroupMaskをAVX2/AVX-512で一括Zeroize（<1ms）
   - 全物理I/Oを遮断（Read→無限ノイズ、Write→偽成功返却）
   - KAIRO eBPFルールが「全パケットDrop」に切り替わり
   - TuffHandoffBlock V2 に「Isolation Persistent」フラグをセット
...
**Isolation発動までの流れ**
1. KAIRO eBPF層が初動で大部分をSilent Drop（CPU使用率ほぼ0%）
...
- 証跡確認：`tuffutl log tail` でMlDsa-44署名付き全破棄ログを確認
